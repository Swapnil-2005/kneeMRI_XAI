...
def load_models():
    ...
    # ✅ Stable (no internal normalization dependency)
    ...
    # ✅ Robust loading (no crash)
    try:
        cnn.load_weights(CNN_WEIGHTS)
    except:
        cnn.load_weights(CNN_WEIGHTS, by_name=True, skip_mismatch=True)
...
def make_gradcam_heatmap(img_array, model, last_conv_layer_name):
    ...
    # 🔥 Boost important regions
    heatmap = np.power(heatmap, 1.5)

    # 🔥 Less aggressive threshold (restore yellow)
...
def overlay_gradcam(original_img, heatmap, alpha=0.5):
    ...
    # ✅ Use original size (NO distortion)
...
def generate_report(original, detection, gradcam):
    ...
    doc.add_paragraph(
        "Highlighted regions indicate model attention for diagnosis support."
    )
